...
namespace Unicorn {
    ...
    constexpr auto rx_dollarendonly    = Flagset::value('d');  // $ does not match line break at end               PCRE_DOLLAR_ENDONLY
    ...
    constexpr auto rx_prefershort      = Flagset::value('S');  // Non-greedy quantifiers, or shorter DFA matches   PCRE_UNGREEDY or PCRE_DFA_SHORTEST
    constexpr auto rx_singleline       = Flagset::value('s');  // Single line mode (. matches anything)            PCRE_DOTALL
    ...
    public:
        RegexError(const u8string& pattern, int error, const u8string& message):
            std::runtime_error(assemble(pattern, error, message)), err(error), msg(message), pat(pattern) {}
        int error() const noexcept { return err; }
        u8string message() const { return msg; }
    ...
    void BasicRegexFormat<CX>::parse() {
        ...
        static const u8string ascii_pattern =
            ...
            R"(|(?:[\\$](.)))";                              // (13) Unknown
        static const regex_type pattern(recode<char_type>(ascii_pattern), rx_optimize);
        size_t prev = 0;
        for (;;) {
