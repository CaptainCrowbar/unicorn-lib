...
words encountered more than once, in descending order of frequency.
    ...
    int main(int argc, char** argv) {
        ...
        try {
            ...
            std::multimap<size_t, Ustring, std::greater<>> frequencies;
...
UTF-8, 16, and 32 strings are represented by `Ustring`, `u16string`, and
...
UTF-16 or 32 are not supported). The name `Ustring` is an alias for
`std::string`; the intention here is that `Ustring` is used for strings
...
an aid to code clarity, and has no effect on the compiler; `Ustring` is just
