...
namespace Unicorn {
    ...
    public:
        ...
        template <typename C> void add(const basic_string<C>& name, C abbrev,
            const basic_string<C>& info, Flagset flags = {},
        ...
        template <typename C> void add(const C* name, C abbrev, const C* info, Flagset flags = {},
            const C* defval = {}, const C* pattern = {}, const C* group = {}) {
                add_option(to_utf8(cstr(name)), to_utf8(basic_string<C>{abbrev}), to_utf8(cstr(info)), flags,
        ...
        template <typename C> void parse(const std::vector<basic_string<C>>& args, Flagset flags = {});
        template <typename C> void parse(const basic_string<C>& args, Flagset flags = {});
    ...
    template <typename C>
    void Options::parse(const std::vector<basic_string<C>>& args, Flagset flags) {
        check_flags(flags);
    ...
    template <typename C>
    void Options::parse(const basic_string<C>& args, Flagset flags) {
        check_flags(flags);
